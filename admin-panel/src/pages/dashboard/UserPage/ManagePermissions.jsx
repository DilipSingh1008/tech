...
const ManagePermissions = () => {
  ...
  // ── Sync perms state when role data or modules load ───────────────────────
  useEffect(() => {
    if (modules.length === 0) return;

    if (permsError) {
      setErrorMsg("Failed to load permissions for this role.");
      const empty = emptyPerms(modules);
      setPerms(empty);
      setOriginalPerms(empty);
      return;
    }

    if (roleData) {
      const list  = roleData?.permissions ?? [];
      const state = apiToState(list, modules);
      setPerms(state);
      setOriginalPerms(state);
      setErrorMsg(null);
    } else if (!loadingPerms) {
      // modules loaded but no role data yet — init empty
      const empty = emptyPerms(modules);
      setPerms(empty);
      setOriginalPerms(empty);
    }
  }, [roleData, modules, permsError, loadingPerms]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveStatus("error");
      setErrorMsg(err?.data?.message ?? err?.message ?? "Failed to save permissions.");
    }
  };
    ...
    <div className="p-6 text-gray-200 font-sans min-h-full">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            onClick={handleSave}
            disabled={saving || isLoading || !roleId}
