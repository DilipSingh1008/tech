...
const VendorPage = () => {
  ...
  const fetchVendors = async () => {
    ...
    try {
      const res = await getItems(
        `vendor?page=${page}&limit=${limit}&search=${searchQuery}&sortField=${sortField}&sortOrder=${sortOrder}`
      );
      setVendors(res.data || []);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      await patchItem(`vendor/toggle-status/${id}`, {});
      setVendors((prev) =>
              ...
              >
                ...
                <Formik
                  ...
                  onSubmit={async (values, { setSubmitting }) => {
                    try {
                      if (editingVendor) {
                        await updateItem(`vendor/${editingVendor._id}`, values);
                      } else {
                        await createItem("vendor", values);
                      }
                  ...
                  {({ values, setFieldValue, isSubmitting }) => (
                    <Form className="space-y-3">
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-(--primary) text-white rounded-lg text-xs font-bold disabled:opacity-60"
                      >
