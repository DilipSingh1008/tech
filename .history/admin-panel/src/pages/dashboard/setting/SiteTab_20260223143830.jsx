...
const SiteSettingsForm = () => {
  ...
  // Single useEffect to fetch data
 useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getItems("setting/site");
        
        // FIX: Check if data is an array and take the first item, 
        // or check the structure from your screenshot
...
console.log(res);
  ...
  const validationSchema = Yup.object().shape({
    ...
    // Note: logo and favicon are not required if we already have them on the server
  });
  ...
  return (
    <div className="max-w-4xl">
      <Formik
        ...
        enableReinitialize={true} // CRITICAL: This allows the form to update when API data arrives
      >
          ...
          <Form className="space-y-6">
            ...
            <div className="pt-6 border-t border-gray-200 dark:border-gray-800 flex gap-4">
              <button
                ...
                className="px-6 py-2.5 cursor-pointer bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-all disabled:opacity-50"
              >
