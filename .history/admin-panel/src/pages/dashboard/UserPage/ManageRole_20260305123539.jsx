...
import { usePatchItemMutation } from "../../../services/api.js";
...
const ManageRole = () => {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    await patchItem({ url: `role/${id}`, data: { status: !currentStatus } });
  };

  // ── Formik Submit ──
  const handleSubmit = async (values) => {
    if (editingRole) {
      await updateItem({ url: `role/${editingRole._id}`, data: values });
    } else {
      await createItem({ url: "role", data: values });
    }
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className={`uppercase tracking-wider font-bold ${theme.header}`}>
                  <tr>
                    ...
                    <th className="px-4 py-3 w-24">Permissions</th>
