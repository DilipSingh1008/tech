...
const ManageServicesPage = () => {
  ...
  const handleToggleStatus = async (id) => {
    try {
      await patchItem({ url: `services/togal/${id}`, data: {} });
    } catch (err) {
            ...
            <table className="min-w-[900px] w-full text-xs">
              <thead className={`uppercase font-bold ${theme.header}`}>
                <tr>
                  ...
                  <th className="px-2 py-3 text-left whitespace-nowrap">SubCategory</th>
                  <th className="px-2 py-3 text-left whitespace-nowrap">Short Desc</th>
                  <th
                    className="px-2 py-3 text-left whitespace-nowrap cursor-pointer hover:text-(--primary) transition-colors"
                  ...
                  <th className="px-2 py-3 text-right whitespace-nowrap">Action</th>
                </tr>
              </thead>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  services.map((s, index) => (
                    <tr key={s._id} className="hover:bg-gray-100 dark:hover:bg-[#1a2030]">
                      <td className="px-2 py-2">{(page - 1) * limit + index + 1}</td>
                      <td className="px-2 py-2 whitespace-nowrap truncate max-w-[150px]" title={s.name}>
                        {s.name}
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap truncate max-w-[120px]" title={s.slug}>
                        {s.slug}
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap">{s.category?.name || "-"}</td>
                      <td className="px-2 py-2 whitespace-nowrap">{s.subCategory?.name || "-"}</td>
                      <td className="px-2 py-2 whitespace-nowrap truncate max-w-[180px]" title={s.shortDescription}>
                        {s.shortDescription}
                      </td>
