...
const ProductCategory = () => {
  ...
  const fetchInitialCategories = async () => {
    try {
      // Using 'categories' as the consistent endpoint
      const res = await getItems("categories");
      const topCats = res.data.filter((cat) => cat.catid === null || !cat.catid);
  ...
  // Fixed Category Change Logic
  const handleCategoryChange = async (categoryId) => {
    // Update category in form and reset subcategory
    setFormData((prev) => ({
      ...prev,
      category: categoryId,
      subCategory: "",
  ...
  const handleEdit = async (prod) => {
    ...
    setFormData({
      ...
      images: [], // Images handle separately or keep existing
    });

    // 2. Fetch subcategories for this product's category
    if (prod.category?._id) {
      try {
        const res = await getItems("categories");
        const filteredSubs = res.data.filter((cat) => cat.catid === prod.category._id);
        setSubCategories(filteredSubs);
      } catch (err) {
        console.error(err);
      }
