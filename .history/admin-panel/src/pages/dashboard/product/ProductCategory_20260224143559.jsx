...
const ProductCategory = () => {
  ...
  // --- Fix: Sync Quill content with formData ---
  useEffect(() => {
    if (quill) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      // Filter only top-level categories (where catid is null)
  ...
  const handleSave = async (e) => {
    ...
    // Ensure last quill content is appended
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`${theme.card} rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col`}>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-6">
                ...
                {/* Main Description (Quill) */}
                <div className={`${theme.section} ${theme.card}`}>
                  <h2 className="text-xs font-bold mb-4">Detailed Description</h2>
                  <div className="mb-4">
                    <label className={theme.label}>Short Summary</label>
                    <textarea className={`${theme.input} h-16`} value={formData.shortDescription} onChange={(e) => setFormData({...formData, shortDescription: e.target.value})} />
                  </div>
                  <div>
