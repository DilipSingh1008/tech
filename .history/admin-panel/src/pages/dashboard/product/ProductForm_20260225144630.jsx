  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        ...
        setCategories(allCats.filter((cat) => !cat.catid));

        // 2. Agar edit mode hai to product load karo
        if (isEdit) {
          const res = await getItems(`product-category/${id}`);
          const prod = res.data;

          console.log("Product data from backend:", prod);

          // 3. Subcategories set karo agar category hai
          if (prod.category?._id) {
            setSubCategories(
              allCats.filter((c) => c.catid === prod.category._id)
            );
          }

          // 4. Form data set karo
          setFormData({
