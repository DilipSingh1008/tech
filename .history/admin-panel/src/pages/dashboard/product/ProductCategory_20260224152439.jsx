...
const ProductCategory = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getItems(`product-category`);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (formData.id) await updateItem(`product-category/${formData.id}`, data);
      else await createItem("product-category", data);
  ...
  return (
    <div className={`h-screen w-full flex flex-col ${theme.main}`}>
      <header className={`px-6 py-4 border-b ${isDarkMode ? "border-gray-800" : "border-gray-200"}`}>
        ...
        <p className="text-[10px] opacity-50">Manage your categories and product details</p>
      </header>
          ...
          <div className={`${theme.card} rounded-xl overflow-hidden`}>
            <table className="w-full text-left text-xs">
              <thead className={`uppercase font-bold ${isDarkMode ? "bg-[#1f2637]" : "bg-gray-100"}`}>
                <tr>
                  <th className="px-6 py-4">ID</th> {/* Added ID Heading */}
                ...
                {products.map((prod) => (
                  <tr key={prod._id} className="hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 font-mono text-[10px] opacity-50">
                      #{prod._id.slice(-6).toUpperCase()} {/* Displayed last 6 chars of ID */}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      <button onClick={() => handleEdit(prod)} className="p-1.5 hover:text-(--primary)">
                        <Edit3 size={15} />
                      </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`${theme.card} rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col`}>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-6">
                <div className={`${theme.section} ${theme.card}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" className={theme.input} value={formData.slug} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} />
                    </div>
                    <div>
                      <label className={theme.label}>MRP *</label>
                      <input type="number" className={theme.input} value={formData.mrp} onChange={(e) => setFormData({ ...formData, mrp: e.target.value })} required />
                    </div>
                    <div>
                ...
                <button type="submit" className="w-full py-3 bg-(--primary) text-white rounded-lg text-sm font-bold hover:brightness-110">
