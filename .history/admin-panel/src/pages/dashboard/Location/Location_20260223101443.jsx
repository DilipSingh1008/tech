                ...
                <tbody className={`divide-y ${theme.divider}`}>
                  {isLoading ? (
                    ...
                    filteredCountries.map((country, index) => (
                      <tr key={country._id} className="hover:bg-(--primary)/5 transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <button
                              ...
                              className="p-1.5 hover:text-red-500 hover:bg-red-500/10 rounded-md transition-all disabled:opacity-30"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4">
          <div className={`${theme.modal} p-5 rounded-xl w-full max-w-xs shadow-xl border border-gray-700/30`}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={closeModal} className="opacity-50 hover:text-(--primary)">
                <FiXCircle size={16} />
              </button>
            </div>
              ...
              {({ errors, touched }) => (
                <Form className="space-y-3">
                  <div>
                    ...
                    <Field
                      name="countryName"
                      className={`w-full p-2 text-sm rounded-lg border outline-none focus:border-(--primary) transition-all ${theme.input} ${errors.countryName && touched.countryName ? "border-red-500" : ""}`}
                    />
                    <ErrorMessage name="countryName" component="span" className="text-red-400 text-[10px] mt-1 block" />
                  </div>
                  <button
                    type="submit"
                    disabled={createLoading || updateLoading}
                    className="w-full py-2 bg-(--primary) hover:opacity-90 text-white rounded-lg text-xs font-bold transition-all disabled:opacity-50"
                  >
                    {editingCountry ? (updateLoading ? "Updating..." : "Update") : (createLoading ? "Creating..." : "Create")}
                  </button>
                </Form>
              )}
            </Formik>
