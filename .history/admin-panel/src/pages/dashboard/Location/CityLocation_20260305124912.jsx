...
const CityLocation = () => {
  ...
  const submitCityFunction = async (values, { resetForm }) => {
    try {
      if (editingCity) {
        await updateItem({
          url: `citylocation/${editingCity._id}/edit-city`,
          data: { city: values.cityName },
        });
      } else {
        await createItem({
          url: `citylocation/${id}/`,
  ...
  const handleToggle = async (cityId, currentStatus) => {
    try {
      await patchItem({
        url: `citylocation/${cityId}/toggle-status`,
                          ...
                          >
                            <span
                              className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${
                                city.status ? "translate-x-5" : "translate-x-1"
                              }`}
                            />
                          </button>
                        </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4">
          <div className={`${theme.modal} p-5 rounded-xl w-full max-w-xs shadow-xl border border-gray-700/30`}>
            ...
            <Formik
              initialValues={{ cityName: editingCity?.name || "" }}
              enableReinitialize
