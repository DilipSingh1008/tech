...
const StateLocation = () => {
  ...
  const submitStateFunction = async (values, { resetForm }) => {
    try {
      if (editingState) {
        await updateItem({
          url: `statelocation/${editingState._id}/edit-state`,
          data: { state: values.stateName },
        });
      } else {
        await createItem({
  ...
  const handleToggle = async (stateId, currentStatus) => {
    try {
      await patchItem({
        url: `statelocation/${stateId}/toggle-status`,
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4">
          <div className={`${theme.modal} p-5 rounded-xl w-full max-w-xs shadow-xl border border-gray-700/30`}>
            ...
            <Formik
              initialValues={{ stateName: editingState?.name || "" }}
              enableReinitialize
                  ...
                  <div>
                    ...
                    <Field
                      ...
                      className={`w-full p-2 text-sm rounded-lg border outline-none focus:border-(--primary) transition-all ${theme.input} ${
                        errors.stateName && touched.stateName ? "border-red-500" : ""
                      }`}
                    />
                    <ErrorMessage
                      name="stateName"
