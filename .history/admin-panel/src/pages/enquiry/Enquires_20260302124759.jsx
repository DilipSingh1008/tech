...
const Enquires = () => {
  ...
  const handleAddSubmit = async () => {
    ...
    try {
      ...
      // await createItem("enquiries", formData);
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  <FiUser size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 opacity-40" />
                  <input
                    type="text"
                    placeholder="Enter your name"
                    value={formData.name}
                    onChange={(e) => { setFormData({ ...formData, name: e.target.value }); setFormErrors({ ...formErrors, name: "" }); }}
                    className={`w-full pl-7 pr-3 py-2 text-sm rounded-lg border outline-none focus:border-(--primary) transition-all ${theme.input} ${formErrors.name ? "border-red-500" : ""}`}
                  />
                </div>
                {formErrors.name && <span className="text-red-400 text-[10px] mt-1 block">{formErrors.name}</span>}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <FiMail size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 opacity-40" />
                  <input
                    type="email"
                    placeholder="Enter your email"
                    value={formData.email}
                    onChange={(e) => { setFormData({ ...formData, email: e.target.value }); setFormErrors({ ...formErrors, email: "" }); }}
                    className={`w-full pl-7 pr-3 py-2 text-sm rounded-lg border outline-none focus:border-(--primary) transition-all ${theme.input} ${formErrors.email ? "border-red-500" : ""}`}
                  />
                </div>
                {formErrors.email && <span className="text-red-400 text-[10px] mt-1 block">{formErrors.email}</span>}
              </div>

              {/* Service */}
              <div>
                <label className="block text-[10px] font-bold opacity-50 uppercase mb-1">Service</label>
                <div className="relative">
                  <FiSettings size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 opacity-40" />
                  <input
                    type="text"
                    placeholder="Enter service name"
                    value={formData.service}
                    onChange={(e) => { setFormData({ ...formData, service: e.target.value }); setFormErrors({ ...formErrors, service: "" }); }}
                    className={`w-full pl-7 pr-3 py-2 text-sm rounded-lg border outline-none focus:border-(--primary) transition-all ${theme.input} ${formErrors.service ? "border-red-500" : ""}`}
                  />
                </div>
                {formErrors.service && <span className="text-red-400 text-[10px] mt-1 block">{formErrors.service}</span>}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full px-3 py-2 text-sm rounded-lg border outline-none focus:border-(--primary) transition-all resize-none ${theme.input} ${formErrors.message ? "border-red-500" : ""}`}
                />
                {formErrors.message && <span className="text-red-400 text-[10px] mt-1 block">{formErrors.message}</span>}
              </div>
