              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className={`uppercase tracking-wider font-bold ${theme.header}`}>
                  <tr>
                    ...
                    <th className="px-4 py-3 cursor-pointer hover:text-(--primary) transition-colors" onClick={() => handleSort("client_id")}>
                      <div className="flex items-center gap-1">Client Name <SortIcon field="client_id" /></div>
                    </th>
                    ...
                    <th className="px-4 py-3 cursor-pointer hover:text-(--primary) transition-colors" onClick={() => handleSort("email")}>
                      <div className="flex items-center gap-1">Email <SortIcon field="email" /></div>
                    </th>
                    <th className="px-4 py-3 cursor-pointer hover:text-(--primary) transition-colors" onClick={() => handleSort("mobile")}>
                ...
                <tbody className={`divide-y ${theme.divider}`}>
                  {isLoading ? (
                    ...
                    <tr>
                      <td colSpan={8} className="px-4 py-10 text-center opacity-40">No enquiries found.</td>
                    </tr>
                  ) : (
                      ...
                      <tr key={enquiry._id} className="hover:bg-(--primary)/5 transition-colors">
                        ...
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-1.5 opacity-70">
                            ...
                            <span className="text-[11px]">{enquiry.email || "N/A"}</span>
                          </div>
                        </td>
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-1.5 opacity-70">
                            <FiPhone size={11} className="shrink-0" />
                            <span className="text-[11px]">{enquiry.mobile || "N/A"}</span>
                          </div>
                        </td>
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-1.5 opacity-70">
          ...
          <div className={`${theme.modal} p-5 rounded-xl w-full max-w-sm shadow-xl border border-gray-700/30`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={`grid grid-cols-2 gap-x-4 gap-y-2 mb-4 pb-4 border-b ${theme.borderT}`}>
              ...
              <span className="flex items-center gap-1.5 text-[10px] opacity-60">
                ...
                {new Date(selectedEnquiry.enquiryDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}
              </span>
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-semibold w-fit ${TYPE_COLORS[selectedEnquiry.type] || "text-gray-400 bg-gray-400/10 border-gray-400/20"}`}>
